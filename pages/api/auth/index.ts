...
export default async function handler(req: NextApiRequest, res: NextApiResponse<AuthResponse>) {
	if (req.method !== "POST")
		res.send({ error: true, msg: "Oups, da ist was schief gelaufen, wir arbeiten daran... 🚧" });
...
async function loginAccount(
	authData: RequestBody
): Promise<{ error: boolean; user: User | null; session: Session | null }> {
