...
function ScheduleDay({dayNumber, date}:ScheduleDay){
    ...
    const [pickedDate, setPickedDate] = dateContext as [ScheduleDate,React.Dispatch<React.SetStateAction<ScheduleDate>>];
    ...
    const setColor = 
    picked ? "bg-gradient-to-b from-sky-300 to-sky-500 text-sky-900" :
    new Date(date?.year as number,date?.month as number,dayNumber).toDateString() === new Date().toDateString() ? 
    "bg-gradient-to-b from-emerald-300 to-emerald-500 text-emerald-900" : "bg-gradient-to-b from-slate-100 to-slate-300 text-slate-900";
