...
function TaskleisteSection({sectionName, index, last = false}:TaskleisteSectionProps){
    ...
    return(
        <div className={"flex justify-around items-center flex-grow "} >
            <span className={`py-4 transition-all ${bookingPageNumber === index || bookingPageNumber > index ? "bg-gradient-to-r from-sky-400 to-emerald-500 bg-clip-text text-transparent" :"text-slate-200" } ${bookingPageNumber === index ? "scale-110" : "scale-100"}  font-bold text-xs xl:text-5xl`}  >{sectionName}</span>
            {
