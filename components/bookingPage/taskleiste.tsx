...
export default function Taskleiste() {
  return (
    <div
      className={
        "p-2 border-slate-800 border-4 flex flex-wrap justify-between xl:p-0 xl:flex-nowrap xl:justify-around "
      }
    >
      {sections.map((e, i) => (
        <TaskleisteSection
...
function TaskleisteSection({
  sectionName,
  index,
  last = false,
}: TaskleisteSectionProps) {
      ...
      <span
        className={`py-4 transition-all ${
          bookingPageNumber === index || bookingPageNumber > index
            ? "bg-gradient-to-r from-sky-400 to-emerald-500 bg-clip-text text-transparent"
            : "text-slate-200"
        } ${
